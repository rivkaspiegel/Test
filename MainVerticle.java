...
public class MainVerticle extends AbstractVerticle {
	...
	// exists Pages (Id integer identity primary key, Name varchar(255) unique,
	// Content clob)";
	...
	//
	// dbClient = JDBCClient.createShared(vertx, new JsonObject().put("url",
	// "jdbc:hsqldb:file:db/wiki")
	...
	//
	// if (res.succeeded()) {
	// List<String> pages = res.result().getResults().stream().map(json ->
	// json.getString(0)).sorted()
	...
	// context.put("id", id);
	// context.put("newPage", fetch.result().getResults().size() == 0 ? "yes" :
	// "no");
